...
- [Component usage](./components.md) - import paths and catalog for every
  exported component
- [5rDesignSystem Guidelines](../src/5rDesignSystem/guidelines/Guidelines.md) - 5R Design System guidelines

## Before building UI

Before writing a custom button, input, card, or any other component, first
check the catalog in `components.md` and the discovery steps in
`setup.md` (Step 3). Prefer kit components. Only hand-roll a custom
replacement when the kit component genuinely cannot meet the requirement,
and leave a code comment explaining why the kit was insufficient.

## Before applying styles

Before hardcoding a color, spacing, radius, font-size, or any other style
value, check the kit's stylesheet (see `setup.md` Step 2) for a CSS
